...
from color_detect import create_color_classifier
...
def detect_vehicle_color(im0, xyxy):
    """
    Detect the color of a vehicle in an image

    Args:
        im0 (numpy.ndarray): Original image
        xyxy (tuple/list): Bounding box coordinates [x1, y1, x2, y2]

    Returns:
        str: Detected vehicle color
    """
    ...
    # Check if the image region is not empty
    if vehicle_img.size == 0:
        return "Unknown"
    
    # Predict color
    try:
        color, confidence = color_classifier.predict(vehicle_img)
        return color
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for j, (*xyxy, conf, cls) in enumerate(reversed(det[:, :6])):
                    # Detect color
                    # Detect color
                    color = detect_vehicle_color(im0, xyxy)

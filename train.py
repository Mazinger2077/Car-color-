...
from utils.general import (LOGGER, check_amp, check_dataset, check_file, check_img_size,
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    if pretrained:
        ...
        try:
            torch.serialization.register_safe_class(Model)
    ...
    hyp['weight_decay'] *= batch_size / nbs  # scale weight_decay
    ...
    if pretrained:
        if resume:
            if isinstance(ckpt, dict):
                best_fitness, start_epoch, epochs = smart_resume(ckpt, optimizer, ema, weights, epochs, hyp, opt.resume)
            del ckpt, csd
    ...
    if RANK in {-1, 0}:
        ...
        callbacks.run('on_pretrain_start')
    ...
    LOGGER.info(f'Image sizes {imgsz} train, {imgsz} val\n'
                ...
                f'Logging results to {colorstr("bold", save_dir)}\n'
                f'Starting training for {epochs} epochs...\n'
                f"Using {compute_loss.__class__.__name__}")
    ...
    for epoch in range(start_epoch, epochs):  # epoch ----
        ...
        if opt.image_weights:
            cw = model.class_weights.cpu().numpy() * (1 - maps) ** opt.evolve / nc  # class weights
            iw = labels_to_image_weights(dataset.labels, nc=nc, class_weights=cw)  # image weights
            dataset.indices = random.choices(range(dataset.n), weights=iw, k=dataset.n)  # rand weighted idx
        LOGGER.info(f"\n{'Epoch':>11}{'GPU_mem':>11}{'box_loss':>11}{'obj_loss':>11}{'cls_loss':>11}{'Instances':>11}{'Size':>11}")
        # Train
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -----------------------------
            ...
            callbacks.run('on_train_batch_end', model, ni, imgs, targets, paths, outputs)
    ...
    if RANK in {-1, 0}:
        ...
        if last.exists():
            strip_optimizer(last)  # strip optimizer from last
        if best.exists():
            strip_optimizer(best)  # strip optimizer from best
            LOGGER.info(f'\nValidating {best}...')
    ...
    # torch.cuda.empty_cache()  # Removed to save time
    return results
